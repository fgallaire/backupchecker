...
class Test1_file_missing_in_tar_gz(Main):
    ...
    def __init__(self, q):
        ...
        self._testdir = 'functional-tests/file-missing-in-tar-gz'
...
class Test2_file_missing_in_tar_bz2(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/file-missing-in-tar-bz2'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file missing in')
...
class Test3_file_missing_in_zip(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/file-missing-in-zip'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file missing in')
...
class Test4_file_missing_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/file-missing-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file missing in')
...
class Test5_wrong_tar_gz_archive_mode(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-mode'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test6_wrong_tar_bz2_archive_mode(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-mode'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected mode')
...
class Test7_wrong_zip_archive_mode(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-mode'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected mode')
...
class Test8_wrong_tar_gz_archive_uid(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-uid'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test9_wrong_tar_bz2_archive_uid(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-uid'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected uid')
...
class Test10_wrong_zip_archive_uid(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-uid'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected uid')
...
class Test11_wrong_tar_gz_archive_gid(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-gid'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test12_wrong_tar_bz2_archive_gid(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-gid'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected gid')
...
class Test13_wrong_zip_archive_gid(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-gid'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected gid')
...
class Test14_wrong_tar_gz_archive_md5_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-md5-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test15_wrong_tar_gz_archive_sha1_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-sha1-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test16_wrong_tar_gz_archive_sha224_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-sha224-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test17_wrong_tar_gz_archive_sha256_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-sha256-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test18_wrong_tar_gz_archive_sha384_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-sha384-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test19_wrong_tar_gz_archive_sha512_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-gz-archive-sha512-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test20_wrong_tar_bz2_archive_md5_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-md5-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test21_wrong_tar_bz2_archive_sha1_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-sha1-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test22_wrong_tar_bz2_archive_sha224_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-sha224-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test23_wrong_tar_bz2_archive_sha256_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-sha256-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test24_wrong_tar_bz2_archive_sha384_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-sha384-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test25_wrong_tar_bz2_archive_sha512_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-tar-bz2-archive-sha512-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test26_wrong_zip_archive_md5_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-md5-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test27_wrong_zip_archive_sha1_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-sha1-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test28_wrong_zip_archive_sha224_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-sha224-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test29_wrong_zip_archive_sha256_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-sha256-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test30_wrong_zip_archive_sha384_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-sha384-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test31_wrong_zip_archive_sha512_hash(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-zip-archive-sha512-hash'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test32_wrong_file_uid_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-uid-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected uid')
...
class Test33_wrong_file_uid_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-uid-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected uid')
...
class Test33_wrong_file_uid_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-uid-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected uid')
...
class Test34_wrong_file_uid_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-uid-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected uid')
...
class Test35_wrong_file_gid_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-gid-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected gid')
...
class Test36_wrong_file_gid_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-gid-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected gid')
...
class Test37_wrong_file_gid_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-gid-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected gid')
...
class Test37_wrong_file_gid_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-gid-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected gid')
...
class Test38_unexpected_file_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/unexpected-file-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test39_unexpected_file_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/unexpected-file-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 unexpected file')
...
class Test40_unexpected_file_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/unexpected-file-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 unexpected file')
...
class Test41_unexpected_file_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/unexpected-file-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 unexpected file')
...
class Test42_wrong_file_md5_hash_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-md5-hash-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test43_wrong_file_sha1_hash_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha1-hash-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test44_wrong_file_sha224_hash_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha224-hash-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test45_wrong_file_sha256_hash_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha256-hash-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test46_wrong_file_sha384_hash_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha384-hash-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test47_wrong_file_sha512_hash_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha512-hash-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test48_wrong_file_md5_hash_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-md5-hash-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test49_wrong_file_sha1_hash_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha1-hash-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test50_wrong_file_sha224_hash_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha224-hash-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test51_wrong_file_sha256_hash_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha256-hash-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test52_wrong_file_sha384_hash_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha384-hash-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test53_wrong_file_sha512_hash_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha512-hash-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test54_wrong_file_md5_hash_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-md5-hash-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test55_wrong_file_sha1_hash_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha1-hash-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test56_wrong_file_sha224_hash_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha224-hash-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test57_wrong_file_sha256_hash_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha256-hash-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test58_wrong_file_sha384_hash_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha384-hash-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test59_wrong_file_sha512_hash_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha512-hash-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test60_wrong_file_md5_hash_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-md5-hash-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test61_wrong_file_sha1_hash_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha1-hash-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test62_wrong_file_sha224_hash_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha224-hash-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test63_wrong_file_sha256_hash_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha256-hash-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test64_wrong_file_sha384_hash_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha384-hash-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test65_wrong_file_sha512_hash_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-sha512-hash-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected hash')
...
class Test66_wrong_file_mode_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-mode-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected mode')
...
class Test67_wrong_file_mode_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-mode-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected mode')
...
class Test68_wrong_file_mode_in_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-mode-in-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected mode')
...
class Test69_wrong_file_mode_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-mode-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected mode')
...
class Test70_wrong_file_type_f_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-type-f-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test71_wrong_file_type_f_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-type-f-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected type')
...
class Test72_wrong_file_type_f_in_zip_archive(Main):
   def __init__(self, q):
       self._queue = q
       self._testname = self.__class__.__name__
       self._testdir = 'functional-tests/wrong-file-type-f-in-zip-archive'
       self._resultfile = os.path.join(self._testdir, 'a.out')
       self._main('1 file with unexpected type')
...
class Test72_wrong_file_type_f_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-type-f-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected type')
...
class Test73_wrong_file_type_d_in_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-type-d-in-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected type')
...
class Test74_wrong_file_type_d_in_tar_bz2_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-type-d-in-tar-bz2-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected type')
...
class Test75_wrong_file_type_d_in_zip_archive(Main):
   def __init__(self, q):
       self._queue = q
       self._testname = self.__class__.__name__
       self._testdir = 'functional-tests/wrong-file-type-d-in-zip-archive'
       self._resultfile = os.path.join(self._testdir, 'a.out')
       self._main('1 file with unexpected type')
...
class Test76_wrong_file_type_d_in_tree(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/wrong-file-type-d-in-tree'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('1 file with unexpected type')
...
class Test77_corrupted_tar_gz_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/corrupted-tar-gz-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
...
class Test79_corrupted_zip_archive(Main):
    def __init__(self, q):
        self._queue = q
        self._testname = self.__class__.__name__
        self._testdir = 'functional-tests/corrupted-zip-archive'
        self._resultfile = os.path.join(self._testdir, 'a.out')
        self._main('data corruption')
